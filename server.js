...
const send = (res, status, body, headers = {}) => { const payload = JSON.stringify(body); res.writeHead(status, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store', ...headers }); res.end(payload); };
const text = (res, status, body, headers = {}) => { res.writeHead(status, { 'content-type': 'text/plain; charset=utf-8', ...headers }); res.end(body); };
...
async function body(req) { const chunks = []; for await (const chunk of req) chunks.push(chunk); if (!chunks.length) return {}; try { return JSON.parse(Buffer.concat(chunks).toString('utf8')); } catch { throw new Error('JSON non valido'); } }
...
function verifyToken(token) { if (!AUTH_SECRET || !token) return null; const [data, sig] = token.split('.'); if (!data || !sig) return null; const expected = crypto.createHmac('sha256', AUTH_SECRET).update(data).digest('base64url'); if (sig.length !== expected.length || !crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expected))) return null; const payload = JSON.parse(Buffer.from(data, 'base64url').toString('utf8')); if (!payload.exp || payload.exp < Date.now()) return null; return payload; }
...
async function routes(req, res, url) {
  ...
  if(bookMatch){const id=decodeURIComponent(bookMatch[1]);const action=bookMatch[2];
    ...
    if(req.method==='GET'&&action==='events'){const {rows}=await pool.query('SELECT * FROM book_events WHERE book_id=$1 ORDER BY created_at DESC',[id]);return ok(res,{events:rows});}
  }
  return notFound(res);
}

const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);if(req.method==='OPTIONS'){res.writeHead(204,{'access-control-allow-origin':'*','access-control-allow-headers':'Content-Type, Authorization','access-control-allow-methods':'GET,POST,PATCH,OPTIONS'});return res.end();}if(url.pathname.startsWith('/api/'))return await routes(req,res,url);return text(res,404,'Passaporto dei Libri API — endpoint non trovato');}catch(e){console.error(e);return send(res,500,{error:e.message||'Errore interno'});}});
